# LYStar Code TUI 易用性与性能后续优化方案

> 文档状态：已完成技术 Review，可按实施清单进入开发
> 评估日期：2026-08-08
> 项目基线：Pi `v0.84.1`，LYStar Code `0.84.1-lystar.6`
...
项目版本以 `packages/coding-agent/package.json` 的 `piConfig` 为事实源：Pi `v0.84.1`、LYStar Code `0.84.1-lystar.6`。本地维护说明中仍出现的旧版本号不作为本方案基线。
...
- `lc -c`、`lc -r`、`lc --help`、`PI_OFFLINE=1 lc --list-models` 的参数和退出码不变。
...
- manifest 版本为 `0.84.1-lystar.6`，Pi 版本为 `0.84.1`，仓库为 `octyean/lystar-agent`。
- Linux x64 解压包的 `lc --version`、`lc --help`、`PI_OFFLINE=1 lc --list-models` 通过。
