...
9. Rust TUI 已按 TypeScript TUI 语义接入 `/fork`、`/session` 和 `/model`：`/fork` 候选来自 Core `getUserMessagesForForking()`，写入复用 Host `fork_session`、lease、journal 和 Session 全窗口锁，成功后恢复 Host 返回的完整原文；`/clone` 继续保持当前分支复制语义。`/session` 复用 Core `getSessionInfo()` 的全量 Session entries 统计，通过受 lease 保护的只读 `get_session_info` 展示名称、文件、消息、Token、费用、Provider/Model 和 Cache 信息；Rust 不读取 JSONL 或按本地 transcript 重算。`/model` 复用既有 `list_models` 与 journaled `set_session_model`，支持当前项定位、Provider/ID/名称筛选和 `/model <provider/model>` 初始筛选，只在严格 snapshot 的 Session path 和模型引用与本次选择一致后提交状态；未认证、空列表、取消、运行中、无 lease、并发写、Host 失败和非法响应均不会伪造模型切换。正式 `lc` 仍默认启动 TypeScript `InteractiveMode`，`/thinking`、认证、剩余设置与项目能力、Shell/Extension parity、真实 Host-Rust 进程 E2E、PTY、完整性能矩阵、三平台 transport 和发行接管尚未完成。
