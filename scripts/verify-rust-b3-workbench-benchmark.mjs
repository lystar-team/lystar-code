...
export function verifyRustB3Workbench(records) {
	...
	for (const row of records) {
		...
		assert(!seen.has(key), `duplicate B3 workbench record ${key}`);
		seen.add(key);
		validateRow(row, key);
	}
...
export function runCli(argv = process.argv.slice(2)) {
	let artifact = ".artifacts/rust-tui-b3-workbench";
	for (let index = 0; index < argv.length; index++) {
		if (argv[index] !== "--artifact") throw new Error(`Unknown argument: ${argv[index]}`);
		artifact = argv[++index];
	}
	const result = verifyRustB3Workbench(readJsonLines(resolve(artifact, "benchmark.jsonl")));
