...
pub(in super::super) fn open_workbench(
    ...
    if target == "thinking" {
        return open_thinking_selector(app, pipe, sequence, session_flow);
    }
    if matches!(target, "login" | "logout") {
        return open_auth_selector(app, pipe, sequence, session_flow, target, String::new());
    }
    if target == "changelog" {
        app.open_overlay(OverlayState::Detail(DetailOverlay {
            title: "更新内容".to_owned(),
    ...
    if target == "help" {
        app.open_overlay(OverlayState::Detail(DetailOverlay {
            ...
            lines: vec![
                ...
                "/settings 设置，/model 模型，/thinking 思考，/login 登录，/logout 退出登录".to_owned(),
