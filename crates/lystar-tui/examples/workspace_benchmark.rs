...
fn benchmark_config() -> BenchmarkConfig {
    let value: Value = serde_json::from_str(
        &fs::read_to_string("benchmarks/rust-workspace-benchmark-scenarios.json").unwrap(),
    )
    .unwrap();
...
fn parse_output_path() -> PathBuf {
    ...
    args.windows(2)
        ...
        .unwrap_or_else(|| PathBuf::from(".artifacts/rust-tui-workspace-benchmark/benchmark.jsonl"))
}
...
fn setup(columns: u16, rows: u16, tool_rounds: usize, regular: bool) -> BenchmarkApp {
    ...
    app.transcript.replace_page(
        ...
        "workspace-active-generation".to_owned(),
        1,
        Some("older-active".to_owned()),
    );
    let mut readonly = ReadonlySessionView {
        path: "/tmp/workspace-readonly.jsonl".to_owned(),
        generation: 1,
        status: "已加载".to_owned(),
    ...
    readonly.transcript.replace_page(
        ...
        "workspace-readonly-generation".to_owned(),
        1,
