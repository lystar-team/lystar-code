...
describe("Web Runtime Protocol v1", () => {
	...
	it("decodes fragmented and coalesced framed messages", () => {
		const first = encodeClientMessage({
			type: "hello",
	...
	it("tracks snapshots, operation order, and transcript gaps", async () => {
		...
		const client = new RuntimeProtocolClient(clientTransport, "client");
	...
	it("decodes a readable version error", () => {
		const [message] = new ServerMessageDecoder().push(
			encodeServerMessage({
				type: "hello_error",
				error: { code: "version", message: "Web Runtime Protocol 2 is unsupported", retryable: false },
			}),
		);
	...
	it("reports the invalid server message kind without serializing its values", () => {
		...
		).toThrow("Invalid Web Runtime server message (event:operation_updated");
	});
