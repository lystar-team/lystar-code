...
export const DiagnosticsSchema = StrictObject({
	...
	runtimeProtocolVersion: Type.Optional(Type.Integer({ minimum: 0 })),
...
const SessionWorkspaceSchema = StrictObject({
	id: Id,
	mode: Type.Union([Type.Literal("shared"), Type.Literal("worktree"), Type.Literal("patch")]),
	projectCwd: Type.String({ minLength: 1 }),
	...
	repositoryRoot: Type.Optional(Type.String({ minLength: 1 })),
	baseCommit: Type.Optional(Type.String({ minLength: 1 })),
...
export const SessionSummarySchema = StrictObject({
	...
	relation: Type.Optional(Type.Union([Type.Literal("collaboration"), Type.Literal("fork")])),
	profileId: Type.Optional(Id),
	profileName: Type.Optional(Type.String({ minLength: 1, maxLength: 4096 })),
...
export const CommandSchema = Type.Union([
	...
	StrictObject({
		command: Type.Literal("room_list"),
		cwd: Type.String({ minLength: 1 }),
		sessionId: Id,
	}),
	StrictObject({
		command: Type.Literal("room_project_list"),
		cwd: Type.String({ minLength: 1 }),
	}),
	StrictObject({
		command: Type.Literal("room_join"),
		cwd: Type.String({ minLength: 1 }),
		roomId: Id,
		sessionId: Id,
	...
	StrictObject({
		command: Type.Literal("room_read"),
		cwd: Type.String({ minLength: 1 }),
		roomId: Id,
		sessionId: Id,
...
export const ServerHelloSchema = StrictObject({
	type: Type.Literal("hello"),
	version: Type.Literal(RUNTIME_PROTOCOL_VERSION),
	productVersion: Type.String({ minLength: 1 }),
