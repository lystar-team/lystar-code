...
const originalScenario = process.env.LYSTAR_WEB_CONTRACT_SCENARIO;
...
afterEach(async () => {
	if (originalScenario === undefined) delete process.env.LYSTAR_WEB_CONTRACT_SCENARIO;
	else process.env.LYSTAR_WEB_CONTRACT_SCENARIO = originalScenario;
...
function createWorkspace(defaultProjectTrust: "always" | "never" = "always"): Workspace {
	const root = mkdtempSync(join(tmpdir(), "web-runtime-contract-"));
	...
	constructor(
		...
		this.child = spawn(process.execPath, args, {
			...
			env: {
				...
				PI_OFFLINE: "1",
				LYSTAR_WEB_CONTRACT_SCENARIO: scenario,
			},
...
async function runWebSessionSwitchContract(workspace: Workspace): Promise<SessionSwitchContract> {
	const targetPath = await createRpcTargetSession(workspace);
	process.env.LYSTAR_WEB_CONTRACT_SCENARIO = "text";
...
async function runWebResourceContract(workspace: Workspace): Promise<ResourceContract> {
	process.env.LYSTAR_WEB_CONTRACT_SCENARIO = "resources";
...
async function runWebToolContract(workspace: Workspace): Promise<ToolContract> {
	process.env.LYSTAR_WEB_CONTRACT_SCENARIO = "tool";
	const adapter = new CodingAgentRuntimeAdapter(workspace.agentDir);
...
function webUiRequest(request: UiRequest): { kind: string; title: string; detail?: unknown } {
...
async function runWebUiContract(workspace: Workspace): Promise<UiContract> {
	process.env.LYSTAR_WEB_CONTRACT_SCENARIO = "text";
	const requests: UiContract["requests"] = [];
	let result: Record<string, unknown> | undefined;
	const runtime = await new CodingAgentRuntimeAdapter(workspace.agentDir).createSession(
		workspace.cwd,
		async (request) => {
			requests.push(webUiRequest(request));
	...
	await runtime.dispose();
	if (!result) throw new Error("Web Runtime 适配器未发出合同通知");
	return { requests, result };
}
...
async function runWebAbortContract(workspace: Workspace): Promise<AbortContract> {
	process.env.LYSTAR_WEB_CONTRACT_SCENARIO = "abort";
...
describe("CodingAgentRuntimeAdapter RPC contract", () => {
	it("matches RPC project resource discovery and Skill/Prompt expansion", async () => {
		...
		const webWorkspace = createWorkspace();
		addProjectResources(rpcWorkspace);
		addProjectResources(webWorkspace);
		const rpc = await runRpcResourceContract(rpcWorkspace, true);
		const web = await runWebResourceContract(webWorkspace);

		expect(web).toEqual(rpc);
		expect(web.commands).toEqual([
			{
		...
		expect(web.userTexts[0]).toBe("Project prompt: alpha beta");
		expect(web.userTexts[1]).toContain('<skill name="contract-project-skill"');
		expect(web.userTexts[1]).toContain("Project skill body");
		expect(web.userTexts[1]).toContain("extra instructions");
	}, 30_000);

	it("matches RPC project trust isolation for resources and commands", async () => {
		const rpcWorkspace = createWorkspace("never");
		const webWorkspace = createWorkspace("never");
		addProjectResources(rpcWorkspace);
		addProjectResources(webWorkspace);
		...
		expect(web).toEqual(rpc);
		expect(web.commands).toEqual([]);
		expect(web.listedSkills).toEqual([]);
		expect(web.userTexts).toEqual([
	...
	it("matches RPC Session switching state and transcript restoration", async () => {
		...
		const web = await runWebSessionSwitchContract(createWorkspace());
	...
	it("matches RPC serializable extension UI request and response semantics", async () => {
		...
		const web = await runWebUiContract(createWorkspace());

		expect(web).toEqual(rpc);
		expect(web.result).toEqual({ selected: "beta", confirmed: true, input: "typed", edited: "edited\ntext" });
	}, 30_000);

	it("matches RPC abort settlement and persisted stop reason", async () => {
		const rpc = await runRpcAbortContract(createWorkspace());
		const web = await runWebAbortContract(createWorkspace());

		expect(web).toEqual(rpc);
		expect(web.transcript.lastAssistantStopReason).toBe("aborted");
		expect(web.events.at(-1)).toBe("phase:idle");
	}, 30_000);
});
