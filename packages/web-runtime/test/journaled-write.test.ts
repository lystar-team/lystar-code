...
function setup() {
	const directory = mkdtempSync(join(tmpdir(), "web-runtime-journaled-write-"));
...
async function connection(service: WebRuntimeService, clientInstanceId = "client", dropResponse = false) {
...
async function lease(service: WebRuntimeService, sessionPath: string, client = "client") {
