...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	protected override beforeTerminalStart(): void {
		...
		const mouseSequence = this.allMouseMotion
			...
			: process.env.TMUX !== undefined ||
				...
				: ENABLE_ALL_MOTION_MOUSE;
		this.terminal.write(
			`${ENTER_ALT_SCREEN}${DISABLE_AUTOWRAP}${this.mouseEnabled ? mouseSequence : ""}${this.clearOnEnter() ? "\x1b[2J" : ""}\x1b[H\x1b[?25l`,
		);
	}
	...
	protected handleViewportInput(data: string): TuiInputListenerResult {
		...
		if (mouseEvent) {
			if (this.handleMouseEvent(mouseEvent)) return { consume: true };
			return undefined;
		}
	...
	private handleMouseEvent(raw: SgrMouseEvent): boolean {
		...
		if (this.handleSearchMouseEvent(raw)) return true;

		const overlay = this.dispatchMouseToOverlay(event);
		if (!overlay.hit) {
			if (this.handleScrollToEndIndicatorMouseEvent(raw)) return true;
			const scrollbarHandled = this.handleScrollbarMouseEvent(raw);
			if (!this.scrollbarDrag) this.updateScrollbarHover(raw.x, raw.y);
			if (scrollbarHandled) return true;
		} else {
			this.stopScrollbarHover();
		}
		...
		if (overlay.hit) return false;

		if (this.handleRightClickPaste(raw)) return true;
