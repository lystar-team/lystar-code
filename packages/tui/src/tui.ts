...
export class TUI extends Container {
	...
	private terminalModeOptions: TerminalModeOptions = { alternateScreen: false, mouse: false };
	private terminalModesActive = false;
	...
	setTerminalModes(options: TerminalModeOptions): void {
		if (
			this.terminalModeOptions.alternateScreen === options.alternateScreen &&
			this.terminalModeOptions.mouse === options.mouse
		) {
			return;
		}
		if (this.started) this.deactivateTerminalModes();
		this.terminalModeOptions = { ...options };
		if (this.started) {
			this.activateTerminalModes();
			this.requestRender(true);
		}
	}

	private activateTerminalModes(): void {
		if (this.terminalModesActive) return;
		let sequence = "";
		if (this.terminalModeOptions.alternateScreen) sequence += "\x1b[?1049h";
		if (this.terminalModeOptions.mouse) sequence += "\x1b[?1000h\x1b[?1006h";
		if (sequence) this.terminal.write(sequence);
		this.terminalModesActive = true;
	}

	private deactivateTerminalModes(): void {
		if (!this.terminalModesActive) return;
		let sequence = "";
		if (this.terminalModeOptions.mouse) sequence += "\x1b[?1006l\x1b[?1000l";
		if (this.terminalModeOptions.alternateScreen) sequence += "\x1b[?1049l";
		if (sequence) this.terminal.write(sequence);
		this.terminalModesActive = false;
	}
	...
	stop(): void {
		...
		// Inline mode leaves the final frame in scrollback. Alternate-screen mode
		// restores the primary screen instead, so moving below the frame would scroll it.
		if (!this.terminalModeOptions.alternateScreen && this.previousLines.length > 0) {
	...
	private doRender(): void {
		...
		const fullRender = (clear: boolean): void => {
			...
			if (clear) {
				...
				buffer += this.terminalModeOptions.alternateScreen ? "\x1b[2J\x1b[H" : "\x1b[2J\x1b[H\x1b[3J"; // Inline redraw also clears scrollback.
			}
