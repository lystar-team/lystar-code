...
import { type ByteTransport, GuiProtocolClient, type OperationSnapshot } from "@lystar/code-gui-protocol";
import { probeIpcHost } from "./ipc.ts";

const SERVICE_NAME = "lystar-gui-host";
...
function launchDaemonLabel(): string {
	return `com.lystar.gui-host.${process.getuid?.() ?? 0}`;
}
...
function windowsTaskName(): string {
	return "LYStar GUI Host";
}

function linuxStatus(endpoint: string): HostServiceStatus {
...
function macStatus(endpoint: string): HostServiceStatus {
	...
	return {
		...
		...(!existsSync(servicePath)
			? {
					...
					remedy: "在远端终端运行 ~/.local/bin/lystar-gui-host install --interactive-admin。",
				}
			: {}),
	};
}

function windowsStatus(endpoint: string): HostServiceStatus {
...
export async function getHostServiceStatus(endpoint: string): Promise<HostServiceStatus> {
	const base =
	...
	const reachable = (await probeIpcHost(endpoint)).reachable;
...
function installLinux(endpoint: string): void {
	...
	const unit = `[Unit]\nDescription=LYStar Code GUI Host\nAfter=network.target\n\n[Service]\nType=simple\nExecStart=${command}\nEnvironment=${JSON.stringify(`PI_GUI_HOST_ENDPOINT=${endpoint}`)}\nRestart=on-failure\nRestartSec=2\nKillMode=mixed\n\n[Install]\nWantedBy=default.target\n`;
...
function installMac(endpoint: string, interactiveAdmin: boolean): void {
	...
	if (!install.ok) {
		throw new Error(
			`macOS 远端后台需要管理员批准：请在远端终端运行 ~/.local/bin/lystar-gui-host install --interactive-admin。${install.stderr || install.stdout}`,
		);
	}
...
async function waitUntilReachable(endpoint: string, timeoutMs = 10_000): Promise<void> {
	...
	while (Date.now() < deadline) {
		if ((await probeIpcHost(endpoint)).reachable) return;
		await new Promise((resolve) => setTimeout(resolve, 100));
	}
	throw new Error("GUI 后台服务启动超时");
}
...
async function waitUntilUnreachable(endpoint: string, timeoutMs = 10_000): Promise<void> {
	...
	throw new Error("GUI 后台服务停止超时");
}

export async function installHostService(endpoint: string, interactiveAdmin = false): Promise<HostServiceStatus> {
	...
	return getHostServiceStatus(endpoint);
}

export async function ensureHostService(endpoint: string): Promise<HostServiceStatus> {
	const status = await getHostServiceStatus(endpoint);
	if (status.reachable) return status;
	if (!status.installed) {
		throw Object.assign(new Error("GUI 后台服务尚未安装"), {
	...
	if (!start.ok) throw new Error(`无法启动 GUI 后台服务：${start.stderr || start.stdout}`);
	await waitUntilReachable(endpoint);
	return getHostServiceStatus(endpoint);
}
...
async function readHostSnapshot(endpoint: string): Promise<HostSnapshot | undefined> {
	if (!(await probeIpcHost(endpoint)).reachable) return undefined;
	const client = new GuiProtocolClient(
	...
	try {
		...
		if (!client.getSnapshot().connected) throw new Error("GUI 后台握手超时");
...
export async function stopHostService(endpoint: string, force: boolean): Promise<HostServiceStatus> {
	...
	if (!force && snapshot) {
		...
		if (active.length > 0 || snapshot.pendingUiRequests.length > 0) {
			throw Object.assign(new Error("GUI 后台仍有运行任务或待处理交互"), {
				code: "host_busy",
	...
	if (!stop.ok && (await probeIpcHost(endpoint)).reachable) {
		throw new Error(`无法停止 GUI 后台服务：${stop.stderr || stop.stdout}`);
	}
	await waitUntilUnreachable(endpoint);
	return getHostServiceStatus(endpoint);
}

export function startDetachedHost(endpoint: string): void {
	const invocation = executableInvocation();
	const child = spawn(invocation.program, [...invocation.args, "serve"], {
		detached: true,
		stdio: "ignore",
		env: { ...process.env, PI_GUI_HOST_ENDPOINT: endpoint },
	});
	child.unref();
}

export function removeHostService(): void {
