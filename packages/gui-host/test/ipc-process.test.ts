...
const hostFixture = fileURLToPath(new URL("./fixtures/ipc-host-worker.mjs", import.meta.url));
...
function startRelay(agentDir: string, endpoint: string): ChildProcessWithoutNullStreams {
	const child = spawn(process.execPath, ["--import", tsxImport, cliPath, "connect", "--stdio"], {
		cwd: repositoryRoot,
		env: { ...process.env, PI_CODING_AGENT_DIR: agentDir, PI_GUI_HOST_ENDPOINT: endpoint },
