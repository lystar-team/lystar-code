...
function formatDecision(trustPath: string | undefined, decision: ProjectTrustStoreEntry | null): string {
	...
	const label = decision.decision ? "信任" : "不信任";
...
export class TrustSelectorComponent extends Container {
	...
	constructor(options: TrustSelectorOptions) {
		...
		this.addChild(new Text(theme.fg("accent", theme.bold("项目可信状态")), 1, 0));
		this.addChild(new Text(theme.fg("muted", options.cwd), 1, 0));
		this.addChild(new Spacer(1));
		this.addChild(
			new Text(
				theme.fg("muted", `已保存：${formatDecision(this.trustOptions[0]?.savedPath, options.savedDecision)}`),
				1,
				0,
			),
		);
		this.addChild(new Text(theme.fg("muted", `当前会话：${options.projectTrusted ? "信任" : "不信任"}`), 1, 0));
		this.addChild(new Spacer(1));
