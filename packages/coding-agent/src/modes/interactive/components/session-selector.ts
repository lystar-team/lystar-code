...
class SessionSelectorHeader implements Component {
	...
	render(width: number): string[] {
		const title = this.scope === "current" ? "继续会话（当前目录）" : "继续会话（全部）";
		const leftText = theme.bold(title);

		const sortLabel = this.sortMode === "threaded" ? "按分支" : this.sortMode === "recent" ? "最近" : "相关度";
		...
		if (this.loading) {
			...
			scopeText = `${theme.fg("muted", "○ 当前目录 | ")}${theme.fg("accent", `加载中 ${progressText}`)}`;
		} else if (this.scope === "current") {
			scopeText = `${theme.fg("accent", "◉ 当前目录")}${theme.fg("muted", " | ○ 全部")}`;
		} else {
			scopeText = `${theme.fg("muted", "○ 当前目录 | ")}${theme.fg("accent", "◉ 全部")}`;
		}
		...
		if (this.confirmingDeletePath !== null) {
			...
			const hint1 =
				keyHint("tui.input.tab", "范围") + sep + theme.fg("muted", 're:<pattern> 正则 · "phrase" 精确匹配');
			const hint2Parts = [
	...
	render(width: number): string[] {
		...
		if (this.filteredSessions.length === 0) {
			...
			if (this.nameFilter === "named") {
				...
				emptyMessage = "  当前目录没有会话。按 Tab 查看全部。";
			}
