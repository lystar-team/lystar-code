...
const tuiModeOption = stringOption("--tui-mode");
...
export const clientCommand = new Command<ClientCommand, ClientCommandContext>("client")
	...
	.build((input) => {
		...
		const tuiMode = input.value(tuiModeOption);
		...
		const tuiModeErrors =
			tuiMode !== undefined && tuiMode !== "regular" && tuiMode !== "fullscreen"
				? [`TUI 模式“${tuiMode}”无效，可选值：regular、fullscreen`]
				: [];
		...
		const unsupportedErrors =
			tuiMode !== undefined
				...
				: input.remainingArgs.length === 0 || prompt !== undefined
					? []
					: unsupportedOptions("client", input);
		const errors = [...authErrors, ...tuiModeErrors, ...modelErrors, ...sessionSelectionErrors, ...unsupportedErrors];
