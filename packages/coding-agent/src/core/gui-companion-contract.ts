...
export const GUI_COMPANION_PROTOCOL_VERSION = 2 as const;
export const GUI_COMPANION_LEGACY_PROTOCOL_VERSION = 1 as const;
export type GuiCompanionProtocolVersion = 1 | 2;

export const GUI_COMPANION_CAPABILITIES = [
	"prompt",
	"steer",
	"follow_up",
	"clear_queue",
...
export type GuiCompanionCapability = (typeof GUI_COMPANION_CAPABILITIES)[number];

export const GUI_COMPANION_LEGACY_CAPABILITIES = [
	"prompt",
	"steer",
	"follow_up",
	"clear_queue",
	...
	"thinking",
	"completion",
] as const satisfies readonly GuiCompanionCapability[];

export interface GuiCompanionSnapshot {
	protocolVersion: GuiCompanionProtocolVersion;
	id: string;
	path: string;
	cwd: string;
	name?: string;
	...
	capabilities: GuiCompanionCapability[];
}

export interface GuiCompanionSnapshotWire extends Omit<GuiCompanionSnapshot, "protocolVersion" | "capabilities"> {
	protocolVersion?: number;
	capabilities?: GuiCompanionCapability[];
}

export type GuiCompanionCommand =
	| { type: "hello"; sessionPath: string; protocolVersion?: GuiCompanionProtocolVersion }
	| {
			type: "request";
			requestId: string;
			command:
