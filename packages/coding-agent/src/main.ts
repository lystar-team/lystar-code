...
async function runAuthCommand(args: string[]): Promise<boolean> {
	...
	if (parsed.unknownFlags.size > 0) {
		...
		console.error(chalk.red(`auth 命令“${getAuthCommandName(command.kind)}”不支持参数 --${option}。`));
		console.error(chalk.dim(`可查看“${APP_NAME} --help”或“${getAuthCommandUsage(command.kind)}”。`));
		process.exitCode = 1;
		return true;
	}
	try {
		...
		const message = error instanceof AuthCommandError ? error.message : "读取凭据失败";
		console.error(chalk.red(`错误：${message}`));
...
async function resolveSessionPath(sessionArg: string, cwd: string, sessionDir?: string): Promise<ResolvedSession> {
	...
	// Exact IDs need only a header lookup; prefixes still require a listing.
	const exactPath = SessionManager.findById(cwd, sessionArg, sessionDir);
	if (exactPath) return { type: "local", path: exactPath };
