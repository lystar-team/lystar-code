...
- 修复 macOS Web 在远程 Git 操作或 Agent Bash 命令中后台等待登录钥匙串弹窗，最终只返回 `mutate_git` 超时的问题；服务升级或安装后的授权改为非阻断后置步骤，只在状态缺失时于本机终端隐藏读取一次登录钥匙串密码并批量授权全部已登记 HTTPS 凭据，30秒无人输入时跳过授权但保留升级结果；Web 在线更新重载后会主动检查并提示待完成授权。
