...
- 合并 Pi v0.87.1，保留 LYStar Code 的中文界面、`lc`/`lystar` 命令、Room 协作、Tool 恢复、扩展事件与发布配置。

### Fixed

- 修正上游合并后的请求预算校验、自动压缩续接和 `/bug` 命令路由。
...
- 修复 Room 输入与普通会话输入并发时共用 Turn 上下文的问题，并延迟仅由 Room 消息唤起的运行时生命周期钩子。

## [0.86.0-lystar.4] - 2026-09-20

### Fixed

- 修复发布打包：跟随 Pi 0.86.0 的剪贴板重构，Unix 与 Windows 打包脚本不再下载 `@mariozechner/clipboard` 原生模块，改为复制 `packages/tui/native/<os>/prebuilds/<platform>` 下的原生辅助模块。
- 修复发布构建：补回合并 Pi 0.86.0 时丢弃的 `FinishReason.TOO_MANY_TOOL_CALLS` 分支，与 `@google/genai` 2.21.0 的枚举对齐。
...
- 新增 Web 会话实时耗时：用户消息下方显示实时「已处理」时长与分隔线，回合结束后由最终回复下方的「本次耗时」接续；计时以客户端按下发送的时刻为起点，未落盘消息完成对账后仍沿用同一时刻，用户看到的最后一次跳动直接作为最终数字。

### Changed

- 调整 Harness 资源迁移为覆盖式：只迁移个人范围的 Skill、子代理、提示词和引用文件，项目规则文件不再作为迁移来源；覆盖前把目标内容备份到 `migration-backups/<时间戳>-<uuid>/<scope>/`，单项失败时恢复该目标，结果返回 `backupPath`；Runtime 协议去掉 `targetScope`、`ruleSelections` 和 `replaceItemIds`，导入项目范围资源时按需请求该项目的信任设置。
- 调整 Harness 资源迁移交互：去掉目标范围切换、规则块合并、Diff 预览和冲突状态，改为先选 Harness 再整体勾选资源，选中全局规则时自动带上被引用文件，其余情况不再单独展示已导入或冲突项。
- 调整 Web 品牌图标：改用带尺寸的 32/48 favicon 和 180/192/512 图标，favicon 不再跟随深浅主题切换。

### Fixed

- 修复 Harness 资源迁移只替换同名文件、遗留旧文件的问题：写入前先清空目标，目录内的陈旧文件和失效脚本不会保留。
- 修复按路径逐条改写文本时前后缀相互覆盖的问题，改为按最长匹配一次性替换。
...
- 修复 macOS Web 在远程 Git 操作或 Agent Bash 命令中后台等待登录钥匙串弹窗，最终只返回 `mutate_git` 超时的问题；服务升级或安装后的授权改为非阻断后置步骤，只在状态缺失时于本机终端隐藏读取一次登录钥匙串密码并批量授权全部已登记 HTTPS 凭据，30秒无人输入时跳过授权但保留升级结果；修复后台读回验证在已经传入用户名时仍要求 Helper 重复输出用户名，导致成功读取密码也被误判失败并清除授权记录的问题；Web 在线更新重载后会主动检查并提示待完成授权。
- 修复应用自更新进程仍使用旧版 `VERSION` 二次 reconcile，导致主程序已升级但 Web Gateway 和 Runtime 长期停留在上一版本的问题；在线更新现在向旧 updater 注入明确的目标版本和原服务版本，当前代码也显式传递更新计划版本，并在首次服务切换回退或版本不符时等待1秒自动重试一次，服务可完成滚动切换，不再依赖手动执行 `lc web service stop` 和 `lc web service start`；更新失败时优先展示安装器的具体错误，不再只显示 `Error: bash 退出码: 1`。
...
- 修复应用更新后 Web 服务状态仍记录旧版本时，`web service start` 和组件启动命令反复拉起旧 Runtime 并以当前协议等待到超时的问题；启动和重启现在检测版本漂移并进入服务版本 reconcile，自动切换到当前应用版本。

## [0.85.1-lystar.10] - 2026-09-12

### Fixed

- 修复 Web 服务从旧 Runtime 协议升级时，空闲检查只使用当前协议并在停止旧服务前失败的问题；现在按旧 Runtime 返回的协议版本读取运行状态，继续阻止活跃任务被升级中断，并允许空闲服务完成自动切换。

## [0.85.1-lystar.9] - 2026-09-12

### Fixed

- 修复 Web Runtime 启动时逐条解释递归 Operation Schema，导致大体量历史操作日志校验超过服务健康检查期限并回退旧版本的问题；现在复用编译校验器，将真实 5.5 MB 日志校验从约 20 秒降至 100 毫秒内，并增加大载荷启动预算回归测试。

## [0.85.1-lystar.8] - 2026-09-12

### Added

- 新增完整 Web Git 工作台，支持多仓库切换、文件树与列表视图、增删行统计、暂存与取消暂存、丢弃改动、提交、Fetch、Pull、Push、分支创建与切换、安全删除、合并与中止合并，以及提交历史分页、提交详情和单文件 Diff 查看。
- 新增已发送 Prompt 编辑能力，可从用户消息操作区恢复文本与图片附件，回到对应会话树节点后重新提交；补充会话重命名、批量删除和项目组拖拽排序。
- 新增 Markdown 文件预览与编辑双视图，并为全局 `AGENTS.md` 设置提供 Monaco Markdown 编辑器、快捷保存、主题同步和响应式行号与缩略图。
- 新增移动端项目与会话抽屉、LYStar Web App 图标和开发环境 Service Worker 清理逻辑，完善桌面安装入口与小屏工作区导航。
- 新增轻量 Prompt 模型目录协议与缓存，只返回已认证模型、Provider 信息、推理能力、上下文窗口和可用 Thinking 等级，并在模型配置变化后主动刷新。

### Changed

- 重构 Web Prompt 编辑器，使用可控富文本输入保留 `@` 文件引用、光标位置、换行、粘贴、输入法组合和附件删除行为；移动端回车换行，桌面端保留快捷提交。
- 调整运行中 Prompt 的发送方式：当前任务可选择“调整方向”或“完成后发送”，普通 Prompt 遇到活跃操作时使用同一队列标识原子降级为 Follow-up，避免重复排队和乐观消息残留。
- 调整会话 Transcript 首屏与实时内容合并，在首个历史页完成前不把实时片段当作完整记录；工具调用、Thinking、图片生成和最终结果统一折叠为“工作过程”，完成态 Diff 增加源码语法高亮并跳过超大文件。
- 调整 Gateway 重连与订阅恢复，复用未丢事件的连接 generation，优先恢复当前会话租约，断线期间保留订阅进度，同时丢弃没有浏览器订阅者的高频进度广播。
- 调整 Web 设置页的外观预览、技能筛选与开关、应用安装、全局指令编辑和更新状态展示，并统一折叠、弹窗与发送操作的 GSAP 动效及减少动态效果偏好。
...
- 修复 Web 迁移导入扫描把规则中提到的目录递归展开为整棵文件树，导致同步文件读取、哈希和路径扫描阻塞界面的问题；现在只迁移允许范围内的显式文件引用，并限制引用链继续解析文档文件。
- 修复 Release Web、源码 Web 与 TUI 共用 Agent 目录时把会话写锁暴露为不可进入状态；恢复独立 development Runtime `2423`，通过版本化 Companion 和 handoff 协议代理或交接单会话写入权，隔离 Runtime Profile、生命周期和操作日志，并在协议升级前保护活跃会话。
- 修复同一会话被多个 Web Runtime 客户端使用时，一个客户端退出会提前释放其他客户端仍在使用的会话；现在按客户端租约和外部 Companion 占用决定所有权、跟随、接管与回收。
- 修复 TUI Resume、Release Web 与 development Web 同时操作会话时的竞争、重复执行和写锁切换问题；Prompt、压缩、重命名及队列任务会等待当前所有者完成，并通过持久化操作日志按客户端请求幂等重放。
- 修复批量删除会话逐条请求、空闲租约未释放和部分失败无法返回的问题；现在单次命令撤销目标会话的空闲 Web 租约，并分别返回删除结果与失败原因。
- 修复模型 Provider 保存后 Prompt 模型列表仍使用旧目录，以及模型设置重复读取造成额外 Runtime 请求的问题。
- 修复 Gateway 重启或 Runtime 断开后错误复用旧恢复 generation、订阅确认前遗漏当前租约、历史读取被会话打开阻塞及未订阅会话进度占用传输的问题。
- 修复已完成 `write` 工具结果只保留统计信息时丢失写入内容，造成 Transcript 和文件差异卡片无法还原实际写入的问题。
- 修复 Web 会话初次加载、队列回执和工具状态交错时出现重复用户消息、空 Thinking、工具间无可见文本、错误 Prompt 重试及图片生成卡片分裂的问题。
- 修复 Git 状态缺少 Detached HEAD、合并中、仓库级统计和 Revision 信息的问题，并限制仓库路径、文件路径、Commit Revision、历史分页及可执行 Git 写操作范围。
- 修复 OpenRouter 图像模型生成器输出未按仓库格式稳定序列化，导致刷新在线模型目录后 `release:local` 被 Biome 阻断的问题。
- 修复 `release:local` 仍按旧 `pi-<platform>` 名称查找 Bun 归档，并假定 Coding Agent 仍导出 `pi` 命令或 CLI 版本等于 npm 包版本，导致无法复制版本化发布包、校验 `lc`/`lystar` 产品版本或生成本地 `pi` 冒烟入口的问题。
- 修复隔离测试、长工作区或深层 Agent 目录使 Unix Socket 路径超过系统限制，导致 Web Runtime 无法启动的问题；超长 Companion 端点会回退到系统临时目录。
...
- 同步 Pi `v0.85.1` 的 Agent、Provider、Tool、TUI、CLI 和 Extension API 更新。
- 保留 LYStar Code 的中文界面、全屏工作区、Web Runtime、Tool Recovery 和 LYStar release 更新源。

### LYStar 适配

- 保留 `lc`/`lystar` 入口、`web`/`web-runtime`/`close-old` 命令和离线启动链路。
- 接入上游模型、Session、Managed Install、全屏搜索和 Tool 渲染变化。

## [0.84.4-lystar.1] - 2026-09-01
...
- Fixed LYStar fullscreen wheel and viewport navigation being consumed by the inherited alternate-screen viewport before reaching the virtual transcript, including after runtime TUI mode switches.
