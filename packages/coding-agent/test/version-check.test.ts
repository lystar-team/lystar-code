...
const TEST_REPOSITORY = "lystar/test-release";
...
describe("version checks", () => {
	...
	it("returns only newer versions", async () => {
		...
		await expect(checkForNewPiVersion("1.2.3", { repository: TEST_REPOSITORY })).resolves.toBeUndefined();
		await expect(checkForNewPiVersion("1.2.2", { repository: TEST_REPOSITORY })).resolves.toEqual({
			version: "1.2.3",
		});
	});

	it("uses the LYStar GitHub release manifest with the product user agent", async () => {
		const fetchMock = vi.fn(async () => Response.json({ version: "1.2.4" }));
		vi.stubGlobal("fetch", fetchMock);

		await expect(getLatestPiVersion("1.2.3", { repository: TEST_REPOSITORY })).resolves.toBe("1.2.4");
	...
	it("returns the active package metadata from the version check api", async () => {
		...
		await expect(getLatestPiRelease("1.2.3", { repository: TEST_REPOSITORY })).resolves.toEqual({
			packageName: "@new-scope/pi",
			version: "1.2.4",
		});
	});
	...
	it("uses the configured repository metadata by default", async () => {
		const fetchMock = vi.fn(async () => Response.json({ version: "1.2.4" }));
		vi.stubGlobal("fetch", fetchMock);
