...
`,
	...
	function mockManagedUpdate(targetVersion: string): void {
		vi.stubGlobal(
			...
			vi.fn(async (input: string | URL | Request) => {
				...
				if (url === "https://github.com/lystar-team/lystar-code/releases/latest/download/release-manifest.json") {
	...
	it("skips untrusted project package settings", async () => {
		...
		try {
			...
			expect(stdout).toContain("没有已安装的 Package。");
			expect(stdout).not.toContain("项目 Package：");
		} finally {
			logSpy.mockRestore();
		}
	});
	...
	it("uses remembered project trust for list", async () => {
		...
		try {
			...
			const stdout = logSpy.mock.calls.map(([message]) => String(message)).join("\n");
			expect(stdout).toContain("项目 Package：");
			expect(stdout).toContain("npm:@project/pkg");
			expect(stdout).not.toContain("没有已安装的 Package。");
	...
	it("blocks local package changes when project is untrusted", async () => {
		...
		try {
			...
			expect(stderr).toContain("项目未受信任。使用 --approve 后才能修改项目 Package 设置。");
	...
	it("updates installer-managed Pi through a staged immutable release", async () => {
		...
		expect(logSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			`已将 LYStar Code 从 ${VERSION} 更新到 ${targetVersion}`,
		);
		expect(errorSpy).not.toHaveBeenCalled();
		expect(process.exitCode).toBeUndefined();
	});
	...
	it("rejects forced managed reinstalls", async () => {
		...
		expect(existsSync(npmRecordPath)).toBe(false);
		expect(errorSpy.mock.calls.map(([message]) => String(message)).join("\n")).toContain(
			"Managed lc installations do not support --force",
		);
		expect(process.exitCode).toBe(1);
	});
...
`,
		...
		try {
			...
			expect(stdout).toContain(`已将 LYStar Code 从 ${VERSION} 更新到 ${VERSION}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
		...
		try {
			...
			expect(recordedArgs).not.toContain(PACKAGE_NAME);
			expect(stdout).toContain(`已将 LYStar Code 从 ${VERSION} 更新到 ${targetVersion}`);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
...
`,
	...
	it("prints a pnpm metadata hint when self-update fails", async () => {
		...
		try {
			...
			expect(stderr).toContain("Run `pnpm store prune` and retry `lc update --self`.");
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
		}
