...
最新补充（2026-08-20）：正式 interactive composition root 已在 Linux x64 接通。Coding Agent `main()` 只接收不依赖 GUI 的 Rust 前端合同，Bun 发行入口组合现有 `AgentSessionRuntime`、同进程 `GuiHostService` 和 Rust child；Runtime adapter 采用 `main()` 已创建的 Runtime，不重新打开 Session，不取得第二个 writer lease。`PI_TUI_FRONTEND=rust|auto` 仅在持久化 Session、无启动消息/图片和无 startup benchmark 时尝试，显式 `rust` 的前置失败会说明原因并在 Session ownership 转移前回退 TypeScript TUI；默认仍为 TypeScript。Unix socket transport 现在承载正式 Host-Rust framed protocol，Host 对内部强类型 `ServerMessage` 直接 CBOR framing，Rust decoder 继续完整校验跨进程消息，消除了 TypeBox 对 200 条 transcript page 的重复深校验；该校验此前稳定耗时约 3.21–3.31 秒，是 150 条 workaround 和 180/200 条超时的实际根因。`EXIT_TRANSCRIPT_PAGE_LIMIT` 已恢复为 200，固定 3 秒等待下，正式 `main()` + 真实 Runtime + 同进程 Host + Rust PTY 的 620 轮/1240 message Session 连续两次完整输出 `needle 0..619`，并验证 `/quit` 复用 release 状态机、terminal mode 恢复和退出顺序；旧 Unix socket 620 轮 E2E 也通过。Rust TUI 全量 147/147 加 transcript 1/1、相关 Vitest 6 files 30/30、GUI Protocol 25/25、Clippy `-D warnings`、Protocol/Host/Coding Agent 构建、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、离线全量构建和 `git diff --check` 均通过。当前环境没有 Bun 1.3.9，`scripts/lystar-bun-cli.mjs` 的 compiled binary 未实际构建；Windows named pipe/ConPTY、macOS、SSH、真实 Provider、正式五平台包装和 release 仍未验证，默认前端尚未切换。
