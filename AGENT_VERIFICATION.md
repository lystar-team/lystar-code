...
最新补充（2026-08-20）：已获授权推送 `feat/rust-tui` 并以 `plan_mode=observe` 在隔离触发分支 `ci/rust-tui-windows-20260820` 跟踪真实 CI；未打 tag、未发布、未合并。功能代码基线为 `00f58e7e7 fix(ci): 收紧必需测试并发`。最终 run `32378843602` 的 `windows-2025` job 完整成功：Coding Agent Windows platform `2/2`、Agent Core Windows platform `1/1`、Rust production named pipe Session lifecycle smoke `1/1`，全部 0 skip；后续 Windows standalone、固定 MinGit 校验、managed Bash、terminal host/icon、Windows PowerShell 5.1 installer、summary 和 artifacts 均成功。`windows-platform-evidence` 包含 `ci-windows-rust-tui-ipc.json` 和 20,996-byte terminal screenshot；platform metrics 记录 wall 328s、build 19s、test 77s，Rust IPC 用例 1533.94ms。真实 runner 首轮暴露并修复了 Windows 同步 named pipe clone 后的读写互锁：Rust 现以共享句柄和 `PeekNamedPipe` 在读取前轮询，不改变 GUI Protocol；同时修复 workflow job-level `if` context、干净 runner build 顺序、Windows 环境变量设置、PowerShell fail-fast、summary JSON 传参和 intentional skip 报告。Linux x64 本地补充验证：Host IPC `2/2`、Rust TUI lib/bin/tests `152/152`、Linux/Windows target Clippy、Windows MSVC check、Coding Agent `--maxWorkers=2` 全量 `2219/2219`、GUI Host required 单 worker `134/134`、脚本 `51/51`、根级 `npm run check` 和 `git diff --check` 通过；完整 Rust all-targets 首次仅在链接 benchmark examples 时遇到本机 lld bus error，随后不含 examples 的全部单元/集成目标通过。最终 workflow 总结仍为 failure：Linux `coding` 在 runner 上触发 lessons-store 多进程 `proper-lockfile ELOCKED`，Linux `core` 的 GUI Host required 仍有 embedded client/server message 校验和 620-record exit transcript decoder 连锁失败，故 `required` 按合同失败；这些不是 Windows job 失败，也没有被降级或忽略。Windows runner 证据不等于 Windows 实机/ConPTY：named pipe ACL/owner、完整交互 `/quit`、中文宽字符、resize、Ctrl+C/窗口关闭、Host/child crash、pipe disconnect 和非 ASCII 安装路径仍待实机。

最新补充（2026-08-20）：Windows required CI 的 Rust TUI production IPC 检查已从 hello-only handshake 扩展为真实 Session 生命周期 smoke。Rust 新增 `--pipe-session-smoke <cwd>` 诊断入口，继续使用正式 `ProtocolPipe` 与 GUI Protocol typed 编解码，依次执行 `create_session -> release_session -> acquire_session -> prompt -> operation completed -> release_session`；Host fixture 记录 prompt 调用并断言 exactly-once。Windows platform job 会先显式 `cargo build -p lystar-tui`，再运行该跨进程测试，避免把首次完整 Rust 编译混入 30 秒 Vitest 用例。Linux x64 本地验证：`ipc-process.test.ts` `2/2`，其中 production IPC smoke `1/1`；脚本 `49/49`；Rust Protocol all-targets `11/11`，Rust TUI all-targets `154/154`，Linux Clippy `-D warnings`、Windows MSVC target Check/Clippy `-D warnings`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、`npm run build:offline` 和 `git diff --check` 均通过；构建生成的 16 个源码旁文件已按差集删除。当前 `feat/rust-tui` 和提交 `1cce8bc24` 尚不在远端，未获推送授权，因此没有 Windows runner 结果；named pipe、Session acquire/prompt/operation/release 的 Windows 运行证据仍待 required CI。

最新补充（2026-08-20）：Rust TUI Windows named pipe production path 已接通到编译与 required CI 合同。`runEmbeddedRustTui()` 不再在 `win32` 直接 fallback；Windows 使用唯一 `\\.\pipe\lystar-rust-tui-<pid>-<uuid>` endpoint，Unix 继续使用私有临时目录和 socket。Rust `ProtocolPipe` 在 Windows 通过标准库双向打开 named pipe，复用现有 frame decoder、hello、request 和 operation contract；早期只因 Unix-only 启动存在的 Composer、Workspace、Extension、Session flow 和 exit output `#[cfg(unix)]` 门闩已移除，真正平台相关条件仅保留在 transport、Unix socket 测试和 fd3/fd4 guard。production IPC handshake 测试改为跨平台，Windows required CI 会实际构建 `lystar-tui.exe`，执行 `serveIpcHost -> named pipe -> Rust --pipe-handshake` 并保存独立 JSON report；现有 workflow 结构测试锁定该命令和报告。Linux x64 本地验证：GUI Host/Rust 聚焦 6 files `65/65`，其中 production IPC handshake `1/1`；Rust Protocol `11/11`，Rust TUI all-targets `154/154`，Linux Clippy `-D warnings` 通过；`cargo check -p lystar-tui --target x86_64-pc-windows-msvc --lib --bin lystar-tui` 和同 target Clippy `-D warnings` 通过；脚本 `49/49`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、`npm run build:offline` 和 `git diff --check` 均通过。构建生成的 16 个源码旁文件已按清单删除。当前没有 Windows runner 或实机结果；named pipe ACL/owner、acquire/prompt/operation、ConPTY、中文宽字符、resize、Ctrl+C/窗口关闭、Host/child crash、安装路径带空格或非 ASCII 仍未验证，默认前端继续保持 TypeScript。

最新补充（2026-08-20）：Rust TUI sidecar 已进入正式五平台发行构建合同，Linux x64 完成真实 package/install smoke。新增 `scripts/build-rust-tui-sidecar.mjs`，只允许在当前原生平台执行 `cargo build --release -p lystar-tui`，并把 `lystar-tui`/`lystar-tui.exe` 复制到 `lc` 相邻目录；`packages/coding-agent` 的 `build:binary`、Unix `build-binaries.sh` 和 Windows `build-windows-release.ps1` 均接入该入口。Unix 构建不再在 Ubuntu 交叉编译四个平台，而由 Release workflow 使用 `macos-15`、`macos-15-intel`、`ubuntu-24.04-arm`、`ubuntu-24.04` 原生矩阵分别产出归档；Windows runner 固定 Rust `1.97.1`。Bun compiled CLI 统一从正式 `scripts/lystar-bun-cli.mjs` composition root 构建；无全局 Bun 时修正为 `npx --yes -p bun@1.3.9 bun`。本机还发现 Bun 1.3.9 直接跨文件系统写 `/tmp` 会返回成功但产生全零 executable，因此 Unix 脚本现在先在仓库 `dist` staging 编译、复制后实际执行 `lc --version`，失败即阻止归档；x64 继续使用 Bun baseline target。打包和安装器均要求 sidecar 存在，归档 SHA/manifest 继续作为唯一完整性事实源。Linux x64 候选归档 `/tmp/lystar-rust-sidecar-release/lystar-agent-v0.84.2-lystar.1-linux-x64.tar.gz` 为 48,994,705 bytes，SHA-256 `478bbb2ce7af2818d9f1dbbc9c5b9b5af4947e09cd98fde0449cbd0ad948a35f`；其中 `lc` 和 `lystar-tui` 都是 x86-64 ELF，`--version`、`lystar --version`、中文 `--help`、`PI_OFFLINE=1 --list-models`、`SHA256SUMS` 通过。解包后不设置 `PI_RUST_TUI_BINARY`，`PI_TUI_FRONTEND=rust` 的真实 tmux PTY 通过 Rust trace 证明自动发现相邻 sidecar；实际安装器覆盖安装、`current/previous`、双向 rollback、篡改 staging 不切换当前版本，以及安装后 `80x8 -> 120x36` resize、`/quit`、退出码 0 和 `stty -g` 恢复。脚本全量 `49/49`、聚焦 Vitest 6 files `80/80`、Rust Protocol `11/11`、Rust TUI `154/154`、Clippy `-D warnings`、Unix 安装器、`NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、`npm run build:offline` 和 `git diff --check` 均通过；构建生成的 16 个源码旁文件和临时 sidecar 已按差集删除。脚本门禁同时修复了既有 Workspace benchmark verifier 文件名笔误。macOS 两架构、Linux ARM64 和 Windows x64 当前仅完成构建/校验配置，尚无本轮 runner 或实机结果；该阶段 Windows named pipe/ConPTY 尚未接入，当前 named pipe 已完成编译与 CI 合同但仍无 Windows 运行证据，默认前端仍是 TypeScript。

最新补充（2026-08-20）：真实 compiled Bun `lc` 与 Linux Rust sidecar 发现链已完成。当前 shell 仍无全局 Bun，使用 `npx --yes -p bun@1.3.9` 提供项目固定 Bun `1.3.9`；`build:binary` 编译 2080 个模块，current HEAD 产出的 `packages/coding-agent/dist/lc` 为 Linux x64 ELF，SHA-256 `9c7e83bc91f562b89177aecde4a61442430ddf3948138122fa4953885f0cb46a`，版本 `0.84.2-lystar.1`。`--version`、中文 `--help`、`PI_OFFLINE=1 --list-models` 通过；手工相邻放置 `lystar-tui` 后自动发现通过，`PI_RUST_TUI_BINARY` wrapper marker 证明显式覆盖优先，移走全部候选后明确打印 acquire 前回退 TypeScript TUI 的诊断。真实 PTY 覆盖手工输入与 faux 流式回复、初始文本、真实 640x537 PNG 加文本、无文本参数的图片输入意图、两条额外 message 顺序与各一次执行、piped stdin 进入 print mode 且不启动 sidecar、620 轮退出 transcript 完整包含 `needle 0..619`、fullscreen/regular、`80x24`、`120x36`、`80x8 -> 120x36` resize、`/quit` 和 `stty -g` 恢复。CLI `@image` 无文本参数仍按既有 file-processor 语义附带空文件标签元数据，不是协议层字面空文本；相邻 sidecar 仅为验证手工放置，完成后已删除，发行包尚未物化 sidecar。binary 构建还暴露 `dist/package.json` 在后续 Node/Vitest 中遮蔽源码包根，导致主题和 HTML 模板解析为 `dist/dist`；提交 `614ae00` 修正 Node 包根识别并增加路径回归。配置 18/18、构建后配置与 Runtime adapter 38/38、Rust frontend/main/adapter/protocol 聚焦 62/62、Host-Rust startup/layout E2E 2/2、Rust Protocol 11/11、Rust TUI 154/154、Clippy `-D warnings`、`cargo fmt --check`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、`npm run build:offline` 和 `git diff --check` 均通过。`build:binary` 的在线模型刷新和 tsgo 生成的 16 个源码旁 `.js/.d.ts/.map` 已恢复或删除。证据仍仅为 Linux x64；五平台归档 sidecar、安装/升级/回滚、Windows named pipe/ConPTY、macOS、SSH、真实外部 Provider 和 Release 未验证，默认前端仍是 TypeScript。

最新补充（2026-08-20）：正式 Rust TUI ownership/lifecycle 故障矩阵已在 Linux x64 补齐。`runEmbeddedRustTui()` 现在把 Host 临时目录创建作为 acquire 前可回退错误处理，并在 server close、Host dispose 或目录删除任一步失败时仍继续执行其余清理；`CodingAgentRuntimeAdapter` 在初始 Runtime 已被取走但 `CoreRuntimeSession.bind()` 失败时会释放 Runtime，不再泄漏 Session writer lock。受控 sidecar fixture 覆盖 sidecar 缺失/不可执行、endpoint 创建失败、child spawn 失败、acquire 前退出、hello 版本拒绝、畸形 framing、acquire 后正常/非零/signal 退出、Host bind 失败，以及 accepted Shell operation 重放后断连；每条链均核对 Runtime factory 未增加、writer lock、endpoint/socket 目录和 exactly-once marker。正式 `main()` PTY 另证明 sidecar acquire 后以 17 退出时，`lc` 返回 17、释放 writer lock 和 endpoint，且不打印 fallback 提示或启动 TypeScript TUI。聚焦 Vitest 6 files `64/64`，Host-Rust startup/layout E2E `2/2`，Rust PTY terminal guard 的 fullscreen/regular × EOF/panic/SIGINT/SIGTERM `8/8`，Rust Protocol `11/11`，Rust TUI `154/154`，Clippy `-D warnings`、`cargo fmt --check`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、`npm run build:offline` 和 `git diff --check` 均通过。当前证据仍仅为 Linux x64；Windows named pipe/ConPTY、macOS、SSH、真实 Provider、compiled Bun `lc`、五平台包装和 Release 未验证，默认前端仍是 TypeScript。

最新补充（2026-08-20）：正式 interactive composition root、启动输入与 TypeScript TUI 外观对齐已在 Linux x64 收口。Coding Agent `main()` 仍只接收不依赖 GUI 的 Rust 前端合同，Bun 发行入口组合现有 `AgentSessionRuntime`、同进程 `GuiHostService` 和 Rust child；Runtime adapter 采用 `main()` 已创建的 Runtime，不重新打开 Session，不取得第二个 writer lease。启动文本、管道 stdin、图片和后续消息现在统一构造成有序 `startupInput`，只在 Rust 成功 `acquire_session` 后随 acquire result 交付；Rust 使用稳定 `startup:<batchId>:<index>` request ID 逐条走正常 `prompt`、operation journal 与终态推进，不经 argv、环境变量或直接 `runtime.prompt()`。纯图片启动会生成携带图片的空文本 prompt；响应丢失 E2E 已证明首条只执行一次且第二条等待首条终态后提交。Rust 进程在 acquire 前退出会返回 `handled: false`，现有 Runtime 不重建且 writer lock 保留给 TypeScript fallback；acquire 成功后由 Rust 接管并在清理时释放，接管后不允许启动第二个前端。界面按当前 TypeScript TUI 重做为两行顶部工作区栏、两列正文边距、独立活动行、`LYStar Code` composer 框、模型/思考底边和同文案快捷栏；上下文百分比来自 Core `getContextUsage()` 投影，不由 Rust 估算。旧 packed footer、正文软件滚动条和 `|` 软件光标已删除，输入使用 Ratatui 硬件光标。活动思考在 assistant 输出前显示 90ms 文本扫描，`reduceMotion` 通过正式 launch option 传入并改为静态文本。真实 Host-Rust PTY 已重新覆盖 `80x24`、`120x36`、`80x8` 和 Thinking 动态帧；正式 `main()` + 真实 Runtime + 同进程 Host + Rust PTY 的 620 轮/1240 message Session 单独复跑通过，完整输出 `needle 0..619` 并通过 `/quit` 恢复终端。Rust TUI lib `151/151`、transcript 集成 `1/1`、benchmark 自测 `2/2`、Rust Protocol `11/11`、聚焦 Vitest 5 files `51/51`、startup/layout E2E `2/2`、Clippy `-D warnings`、`cargo fmt`、schema 再生成前后 SHA-256 一致、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、离线全量构建和 `git diff --check` 均通过。当前环境仍没有 Bun 1.3.9，`scripts/lystar-bun-cli.mjs` compiled binary 未实际构建；endpoint 创建失败、hello/protocol 拒绝、接管后 Rust/Host 崩溃、signal/terminal disconnect 的完整 ownership matrix，Windows named pipe/ConPTY、macOS、SSH、真实 Provider、正式五平台包装和 release 仍未验证，默认前端尚未切换。

最新补充（2026-08-19）：全屏退出记录分页已采用保守的 Unix FIFO 单帧上限。`EXIT_TRANSCRIPT_PAGE_LIMIT` 从 200 调整为 150；退出记录专项测试在固定 3 秒 Host 响应等待下连续两轮通过，单页 50、100、150 条可通过，180 和原始 200 条仍会复现大帧读取超时。因此当前结果只证明分页规避有效，不证明 FIFO 大帧传输根因已修复；正式 Unix socket/分片传输、Windows named pipe 和正式 `lc` Rust TUI 启动链仍未验证。

最新补充（2026-08-19）：Rust TUI/CustomEditor 剪贴板附件 E2E 已收口。A `editing-lifecycle` 默认 5,000 字符粘贴连续两轮通过；B `completion-submission` 连续两轮通过，覆盖内置命令、idle prompt、steer、follow-up、abort、response-drop 和 exactly-once；C `CustomEditor` 连续两轮通过，覆盖真实 completion、direct/recovery menu、失败附件缺失保护，以及提交附件 A 后保留后加入附件 B。Rust 新增 `settles_custom_editor_submit_and_keeps_later_attachments` 单测，确认结算只按提交 hash 清理 A，不删除 B。C 的成功提交会产生普通 notify 详情 overlay，测试先按 Escape 关闭 overlay 再读取 Composer，因此区分了“状态已保留但被 overlay 遮挡”和“附件被错误清理”。本轮定向 GUI Host `29/29`、Rust TUI lib `141/141`、transcript `1/1`、benchmark 自测 `2/2`、Rust Protocol `11/11`、Coding Agent build、根级 `npm run check`、Clippy `-D warnings`、fmt 和 `git diff --check` 均通过。证据范围仍仅为 Linux x64、tmux、Unix FIFO/fd3/fd4 与 faux provider；Windows named pipe、Kitty/iTerm2 图片终端、真实外部 Provider、PTY 多尺寸和正式 `lc` Rust TUI 启动链未验证。

最新补充：Rust TUI 已完成 Extension 动态命令与快捷键 parity。Slash 命令目录、参数补全和 `Ctrl+P` 动态命令均复用 GUI Host `RuntimeSession.getCompletions()`；Rust 继续保留内置命令立即显示，Host 返回后合并 Extension 命令。动态命令执行复用 Core `AgentSession.prompt()` 的立即执行语义，流式 `steer()` / `followUp()` 对已注册 Extension command 也转回 `prompt()`，不把它们错误排入 Agent 队列。动态快捷键由 Host 使用公开 Core `KeybindingsManager`、Extension `getShortcuts()` 和 Pi TUI `matchesKey()` 判定；Rust 只有在 Host 状态报告存在快捷键时才转发原始按键，未命中仍回退本地输入。命令面板响应的查询文本与 Composer 草稿分离，动态候选写回时替换当前草稿，不再用 `"/"` 查询偏移切空字符串。新增真实动态 Extension fixture 和 Linux x64 Host-Rust tmux/FIFO E2E，连续两轮覆盖 Slash 补全、参数 `alpha` 补全、Extension command 执行、Ctrl+P 动态目录和 `Ctrl+Shift+U` 原始快捷键转发。本轮定向结果：动态 E2E `1/1`（内部两轮）、GUI Host 非 E2E 全量 `116/116`（另有 Runtime/bridge 聚焦 `36/36`）、GUI Protocol `24/24`、Rust TUI all-targets `143/143`、Rust Protocol all-targets `11/11`；Clippy `-D warnings`、fmt、`git diff --check` 均通过。GUI Host 含全部 `rust-tui-e2e.test.ts` 的全量 Vitest 在 600 秒内未完成，不能视为通过；本轮只把动态 Extension 场景作为独立 `1/1` 证据，未把其余 E2E 套件误写成通过。真实 Provider、完整 45/330 benchmark、PTY 多尺寸专项、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链仍未在本轮验证。

最新补充：Rust TUI 已完成 Shell `!` / `!!` parity。`!command` 通过 GUI Protocol `run_bash`、Session lease 和 Host operation journal 调用 Coding Agent Core `executeBash()`，结果写入 Session 并进入模型上下文；`!!command` 使用同一路径并显式发送 `excludeFromContext: true`，Core 持久化 Bash 记录但从模型上下文排除。Host Runtime adapter 先触发 `user_bash` Extension hook，Extension 返回完整结果时由 adapter 流式投影并调用 `recordBashResult()`，否则将 Extension 自定义 operations 交给 Core 执行。实时 operation progress 使用结构化 Bash 命令和累计输出，Host 仅保留最新 16 KiB 并标记截断；Rust 复用现有 `LiveBash`，Session `bashExecution` 提交后由 transcript 记录替换实时尾部，不重复显示。并发任务、等待 accepted、无 lease、附件、发送失败、Host 拒绝、非法响应、执行失败、取消和中断均不启动第二条 Shell；失败终态恢复原 `!` / `!!` Composer 草稿，Esc 继续复用 `abort_operation -> RuntimeSession.abort() -> AgentSession.abortBash()`。共享协议新增 `excludeFromContext` 必填字段；GUI 现有 `!` 请求显式发送 `false`，本项未扩展 GUI `!!`。本轮 Rust Shell 专项 `4/4`、Rust TUI 全目标 `143/143`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host journal + Runtime adapter + observation `73/73`；全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host -> GUI 顺序构建、schema 与 TS/Rust golden fixture 生成、根级 `npm run check` 和 `git diff --check` 均通过。本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport、GUI `!!` 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已完成 Skill、Package 和项目/本机指令写入 parity。现有 `/skills`、`/packages`、`/instructions` List/Editor Overlay 继续复用 Host/Core 的结构化资源接口；Rust 写请求现在携带当前 `sessionPath` / `leaseId`，运行中、并行 Session flow、已有写入或无 lease 时不发请求。共享 GUI Protocol 中两个控制字段保持可选，无活动 Session 的桌面设置页仍可管理全局资源；Host 只在收到控制字段时强制成对校验 lease、受控 Runtime 和 canonical cwd，并用 operation journal 与 Session 全窗口锁覆盖“配置写入 + 相关 Runtime 资源重载”。成功响应新增 Skill 目标回显和 Package 刷新列表，Rust 严格核对路径、作用域、目标值、指令内容及 Package 安装/移除后的列表状态后才更新缓存；Host 失败、重载失败和非法响应保留 Overlay、筛选、编辑内容与 Composer 草稿。Skill/指令配置和 Package 安装器副作用在重载失败时不做无证据回滚，journal 如实记录失败，下一次刷新以磁盘与安装器结果为准。本轮 Rust 资源 mutation 专项 `3/3`、Rust TUI 全目标 `139/139`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `66/66`；全目标 Clippy `-D warnings`、fmt、正式 Rust TUI binary check、Protocol -> Coding Agent -> GUI Host 顺序构建、schema 与 TS/Rust fixture 生成、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。CodeGraph affected 还包含明确延期的真实 `rust-tui-e2e.test.ts`；本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、真实安装器外部副作用、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已完成 Project Trust parity。首次进入项目继续复用 Core `resolveProjectTrusted()` 与通用 `ui_request`，支持信任当前目录、信任上级目录、仅本次会话信任、不信任和取消；Rust 不复制 Project Trust 判断。已进入会话后的 `/trust` 继续使用 `get_project_trust` / journaled `set_project_trust`，写入协议新增当前 `sessionPath` / `leaseId`，Host 要求受控 Runtime 的 canonical cwd 与目标项目一致，并在“写 trust + 重载同 cwd 全部 Runtime”窗口锁住当前 Session。Runtime adapter 区分项目直接决定和父目录继承决定；资源重载失败时 Host 恢复写前的直接决定并再次重载，journal 记录失败。Rust 只有在 response 的 cwd 和 trusted 与请求完全一致时才更新状态；取消、运行中、并行 Session flow、并发写、无 lease、Host 失败和非法响应均保留旧状态、Overlay 和 Composer 草稿。Protocol semantic fixture 已同步 TS/Rust 双向 golden frame。本轮 Rust Project Trust 专项 `3/3`、Rust TUI 全目标 `136/136`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `65/65`；全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建和 `git diff --check` 已通过。真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链未运行。

最新补充：Rust TUI 已完成设置与主题选择 parity。`/settings` 继续复用 Host `list_settings` / `set_setting`、Core `SettingsManager` 和现有 Session lease、operation journal、全窗口写锁；Rust 不读取或写入 `settings.json`。Host 只投影 `getLystarSettingsForUi()`，枚举 `options` 保留提交原值，`optionLabels` 使用 Core descriptor `format()` 的显示文案。主题候选由内置 `dark`/`light` 与当前 Session `resourceLoader.getThemes()` 合并，不读取进程级全局 Theme registry；Rust 支持固定主题和现有 `light/dark` 自动组合格式，保存后返回设置列表。当前 Rust 没有动态主题配色预览引擎，本项未增加伪预览。本轮 Rust 设置专项 `1/1`、Rust TUI 全目标 `133/133`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter `13/13`、Coding Agent 设置目录 `5/5`；全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、schema 与 TS/Rust fixture 生成、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check` 和 `git diff --check` 均通过。本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport、主题运行时预览和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已完成动态应用键位、模型前后循环、思考强度循环和思考过程折叠。Rust 将 Crossterm 的 `Ctrl`、`Shift`、`Alt`、`Super` 组合编码为 Pi TUI `matchesKey()` 已支持的传统序列、CSI-u 或 xterm modifier 序列；Host headless editor 继续使用 `KeybindingsManager.create(agentDir)` 读取本次 Runtime 的 `keybindings.json`，Rust 不维护 action 到按键的第二份映射。自定义 `Ctrl+Shift+P`、`Alt+M`、`Shift+Tab`、默认控制字符和冲突优先级均由同一 Core 键位实现判定。`app.model.cycleForward`、`app.model.cycleBackward` 通过新的 journaled `cycle_session_model` 调用 Core `AgentSession.cycleModel()`，保留 scoped model、认证可用性、模型顺序、思考等级继承/钳制和持久化语义；`app.thinking.cycle` 通过 `cycle_session_thinking` 调用 Core `cycleThinkingLevel()`，响应区分不支持与单一支持等级的同值循环。两条循环命令继续使用 Session lease、Host operation journal、全窗口写锁和正式 snapshot；运行中、无 lease、并行 Session flow 或已有写入时不发请求，等待 response 时延迟同 Session 提前广播，非法或语义不一致结果不提交。`app.thinking.toggle` 只切换 Rust 本地 `hide_thinking`，同时折叠已提交和流式思考过程，不修改 Session thinking level。Protocol semantic fixture 已补齐 TS/Rust 双向 golden frame。本轮 Rust TUI lib `129/129`、全目标 `132/132`，Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Extension bridge + Runtime adapter + journal `72/72`、Coding Agent Core 键位冲突与模型能力 `13/13`；全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、schema 和 TS/Rust fixture 生成、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。CodeGraph affected 指向模型、思考、Host bridge、journal 和 `rust-tui-e2e`；前四类已覆盖。本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、真实外部 Provider、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已接入 Provider `/login`、`/logout` 认证。Provider 目录、认证方式、模型目录、凭据存储和 operation journal 均继续复用 GUI Host/Coding Agent Core；Rust 不读取 `auth.json`、不保存或回显凭据，只通过既有 `ui_request`/`ui_response` 投影 API Key 密码输入、OAuth URL/设备码/等待状态。`/login <provider>` 按 TypeScript TUI 语义精确匹配 Provider ID/名称：单认证方式直接启动，多认证方式再选择；找不到精确项时保留初始筛选。`/logout` 只列出 `authSource=stored` 的 Provider，只删除 `/login` 保存的凭据，环境变量和 `models.json` 不受影响。登录或退出返回的模型目录先作为候选状态，再用 `list_model_providers` 核对目标 Provider、认证方式、`authSource`、模型数量与认证状态；两次响应均严格通过后才提交 Rust 缓存，确认期间保持 `write_pending`。Host 失败、取消、非法响应或语义不一致均恢复认证列表并保留原模型、Provider 缓存和 Composer 草稿；密码提交或 Esc 取消后 Rust 状态不再保留输入值，Esc 明确发送 `cancelled: true`。认证 OAuth 通知限定为单个状态页，且不会抢占活动输入 Overlay；Extension 普通 `notify` 行为不变。旧登录列表内按 `d` 退出的双轨入口已删除。认证入口、门禁、请求和两步结果核对集中在 `terminal/auth.rs`，未修改既有 Protocol、Host 或 Core 契约。本轮 Rust 认证专项 `7/7`；Rust TUI lib `126/126`、全目标 `129/129`，Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `67/67`；全目标 Clippy `-D warnings`、fmt、schema、TS/Rust fixture、Protocol -> Coding Agent -> GUI Host 顺序构建、正式 Rust TUI binary check、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、离线全量构建、CodeGraph sync 和 `git diff --check` 均通过。本轮未运行真实外部 Provider、真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链；纯 OAuth 等待阶段仍缺 operation ID 取消契约，Esc 只能取消交互式 prompt，不能中止已经进入纯等待的 Host 登录。

最新补充：Rust TUI 已接入 `/thinking`。思考等级目录继续读取 GUI Host `list_models` 对当前模型的 Core 能力投影，Overlay 只展示 `supportedThinkingLevels` 声明的等级并默认定位当前项；未认证、不支持推理、空能力、无当前模型或目录缺失时显示明确不可用原因。写入复用受当前 Session lease 保护并进入 operation journal 的 `set_session_thinking`，运行中、无 lease、并行 Session flow 或已有写入时不发请求。Rust 新增专用 `ThinkingMutation`，严格核对 response snapshot 的 Session path、Provider、Model ID 和所选等级；模型或思考写入等待 journal response 时，同 Session 的提前广播 snapshot 会延迟应用，Host 失败、未知字段或语义不一致响应均保留原模型、思考等级、Overlay 和 Composer 草稿。思考 action 已从设置/认证混合处理器移入独立 `terminal/thinking.rs`，未修改既有 Protocol、Host、Core 或认证逻辑。本轮定向测试为 Rust `/thinking` `5/5`；Rust TUI 全目标 `122/122`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `60/60`，全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、正式 Rust TUI binary check、`npm run check:schema`、`npm run generate:rust-fixtures`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已接入 `/model`。模型目录直接复用 GUI Host `list_models` 返回的 Core/Runtime 投影，模型切换继续使用受当前 Session lease 保护并进入 operation journal 的 `set_session_model`，Rust 不维护第二份模型状态。模型 Overlay 支持当前模型默认定位、Provider/ID/名称筛选、`/model <provider/model>` 初始筛选、未认证状态、空列表和 Esc 取消；运行中、无 lease、并行 Session flow 或已有写入时不发切换请求。成功只应用严格 schema 校验且 Session path、Provider、Model ID 与本次选择一致的 snapshot；Host 失败、未知字段或语义不一致响应均保留原模型、Overlay 和 Composer 草稿。模型 action 已从设置/思考混合处理器移入独立 `terminal/model.rs`，未修改既有 Protocol、Host、Core 或 `/thinking`、认证逻辑。本轮定向测试为 Rust `/model` `5/5`；Rust TUI 全目标 `117/117`、Rust Protocol `11/11`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `60/60`，全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、正式 Rust TUI binary check、`npm run check:schema`、`npm run generate:rust-fixtures`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已接入 `/session`。Coding Agent Core 新增共享 `getSessionInfo()`，统一复用全量 Session entries 的消息统计、累计 Token/费用、Provider/Model 归组和 Cache 重复计费口径；TypeScript TUI 已改为读取同一结果。GUI Protocol 新增受当前 Session lease 保护的严格只读 `get_session_info`，Host 只从当前受控 Runtime 转发，Rust 不读取 JSONL、不按本地 transcript 重算；临时 Session 以 `sessionFile: null` 显示“仅存于内存”。Rust 详情 Overlay 对齐 TypeScript `/session` 的名称、文件、ID、消息、工具、Token、费用明细和 Cache 信息，并接入 Slash 精确匹配、Ctrl+P 命令面板和 `/help`；无 lease、Host 失败和非法响应均不会显示伪造结果。本轮定向测试为 Rust `/session` `4/4`、Coding Agent Core stats `9/9`、GUI Protocol `24/24`、GUI Host Runtime adapter + journal `60/60`；Rust TUI 全目标 `112/112`、Rust Protocol `11/11`（含新增 TS/Rust 双向 golden frame）、全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、正式 binary check、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。CodeGraph affected 还指向真实 `rust-tui-e2e.test.ts`；本轮未运行 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI 已接入 `/fork`。候选消息由 GUI Host 通过 Coding Agent Core `getUserMessagesForForking()` 提供，受当前 Session lease 保护；Rust 只保存候选 `entryId`，选择后复用既有 journaled `fork_session` 并固定发送 `position: "before"`。Host 在整个异步 fork 窗口锁住 Session，成功换路径时同步迁移 lease 和 operation lock；同一 request 在换路径后重试仍由 journal 返回既有结果，不重复 fork。Rust 仅在生成 schema 严格校验 lease、snapshot 和 `/fork` 必需的 `selectedText` 后切换 Session 并恢复完整原文；Esc、无消息、运行中、无 lease、并行 Session 操作、Host/Core 取消、失败、非法响应或缺少 `selectedText` 均保留旧 Session、lease、transcript 和 Composer 草稿。命令已加入 Slash 精确匹配、Ctrl+P 命令面板和 `/help`；`/clone` 继续使用 `position: "at"` 且不恢复原文。本轮定向测试为 Rust `/fork` `5/5`、GUI Protocol `23/23`、GUI Host Runtime adapter + journal `59/59`；Rust TUI 全目标 `108/108`、Rust Protocol `11/11`（含新增 TS/Rust 双向 golden frame）、全目标 Clippy `-D warnings`、fmt、Protocol -> Coding Agent -> GUI Host 顺序构建、`npm run check:schema`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。CodeGraph affected 指向 `/fork`、Protocol、Host journal/runtime 和 `rust-tui-e2e`；前三类已覆盖，本轮未运行真实 Host-Rust 进程 E2E、PTY、完整 45/330 benchmark、macOS、Windows、正式 transport 和正式 `lc` Rust TUI 启动链。

最新补充：Rust TUI benchmark examples 已同步当前 transcript 投影类型。`m8_benchmark` 和 `workspace_benchmark` 的 Tool result fixture 现显式提供 `diff`，只读详情投影读取 `TranscriptDetailLine.text`；`cargo clippy -p lystar-tui --all-targets -- -D warnings` 和 `cargo test -p lystar-tui --all-targets` 均通过，其中 Rust TUI lib `100/100`、集成测试 `1/1`、benchmark 自带测试 `2/2`；fmt、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check` 和 `git diff --check` 也通过，先前 benchmark example 阻塞已解除。两个 example 没有小样本参数，本轮未运行 45/330 条完整性能矩阵，未生成或更新 benchmark artifact。

最新补充：Rust TUI 已接入 `/reload`。GUI Protocol 新增受 Session lease 保护的 `reload_resources`，Host 通过 operation journal 调用既有 `RuntimeSession.reloadResources()`，在整个异步重载窗口锁住 Session 并在完成后返回规范 snapshot；并发 Prompt、活动 operation、无 lease 和并行 Session 操作均不会执行重载。Rust 等待成功响应后只应用严格 schema 校验过的 snapshot；失败或非法响应保留当前 Session、lease、transcript 和 Composer 草稿，重载等待期间提交草稿也不会清空输入。命令已加入 Slash 精确匹配、Ctrl+P 命令面板和 `/help`。本轮定向测试为 Rust `/reload` `5/5`、GUI Protocol `22/22`、GUI Host journal `43/43`，Host Runtime adapter + journal `56/56`；Rust TUI lib `100/100`、Rust Protocol `11/11`（含新增 TS/Rust 双向 golden frame）、正式 binary check、正式 lib/tests/bin Clippy `-D warnings`、fmt、三包顺序构建、`npm run check:schema`、根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check`、CodeGraph sync/affected 和 `git diff --check` 均通过。`cargo clippy -p lystar-tui --all-targets -- -D warnings` 当时仍被既有 benchmark example 的 `TranscriptDetailLine` 与缺少 `diff` 字段编译错误阻塞，本轮没有修改 benchmark；该阻塞已由后续独立维护提交解除。PTY/E2E、真实 Host 进程、macOS、Windows 和正式 `lc` 启动链未在本轮验证。

最新补充：Rust TUI 已接入 `/clone`。命令从 Host `SessionStateSnapshot.leafId` 取得当前分支叶节点，通过既有 `fork_session` 发送 `position: "at"`，成功后复用 `SessionFlow::Fork` 和 Host 返回的新 lease/snapshot 切换会话；运行中、并行会话操作、无 lease 和空 Session 均不会发请求，Host 失败时保留原 Session、lease、transcript 和失败后输入的草稿。本轮 `/clone` 定向测试 `4/4`、Rust TUI lib 测试 `95/95`、Rust Protocol `11/11`、正式 binary check、Clippy `-D warnings`、fmt、`git diff --check` 和根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check` 均通过；CodeGraph affected 仅指向 clone/hotkeys 测试。未新增 Core、Host 或 GUI Protocol 命令，PTY/E2E、真实 Host 进程、macOS、Windows 和正式 `lc` 启动链未在本轮验证。

最新补充：Rust TUI 已接入本地只读 `/hotkeys` 详情页，并加入 Ctrl+P 命令面板和 `/help`。页面按导航、编辑、工作台和输入前缀分组，只列 Rust 输入处理器当前实际生效的按键；未展示尚未迁移的 `!`/`!!`，也未新增 Host/Protocol 链路。本轮定向测试 `2/2`、Rust TUI lib 测试 `91/91`、正式 binary check、Clippy `-D warnings`、fmt、`git diff --check` 和根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check` 均通过；CodeGraph affected 仅指向 changelog/hotkeys 测试。该项完成时动态应用键位尚未接入，后续增量已通过 Host `KeybindingsManager` 完成；PTY/E2E、macOS、Windows 和正式 `lc` 启动链未在本轮验证。

最新补充：Rust TUI B4 CustomEditor 综合 E2E 已拆成 Linux x64 的 A/B/C 三个独立 Host-Rust PTY 场景，默认各两轮、开发可用 `LYSTAR_CUSTOM_EDITOR_ATTEMPTS=1`。A 验证草稿、Unicode、多行、5000 字符 paste、历史、stale callback、replace 与 factory failure；B 验证内置命令、idle prompt、active steer、Alt+Enter follow-up、Esc abort 与 response-drop 恰好一次；C 验证原生 Composer 的真实 PNG `/attach`、冻结重试与 CustomEditor `Ctrl+V` 剪贴板图片。每轮显式 Runtime abort、Host dispose、关闭 FIFO，最多 2 秒等 tmux 退出并核对 `stty` 恢复；artifact 只记录计数、长度和 hash，扫描禁止草稿原文、base64 与典型凭据。真实 Pi `border-status-editor`、`modal-editor`、`rainbow-editor` 原例子仍各两轮通过。本项只覆盖 Linux x64、tmux、Unix FIFO/fd3/fd4 和 faux provider；Windows named pipe、Kitty/iTerm2 图片终端与真实外部 Provider 未验证。详见 `docs/rust-tui-b4-extension-report.md`。

最新补充：Rust TUI B4 Extension Component invalidate storm 已在 2026-08-16 的 Linux x64 真正 `CodingAgentRuntimeAdapter -> GUI Host -> fd3/fd4 FIFO -> tmux Rust TUI` 链路完成。`/contract-components-storm` 在约 500ms 实际触发 1000 次 state increment + `requestRender`，每个 `80x24`、`120x36`、`200x60` 各 5 轮，共 `.artifacts/rust-tui-extension-component-storm/benchmark.jsonl` 15 条；final 均为 `1000`，render/publish `28..31`，coalesced `971..973`，idle component frame `0/15`。按全部原始 invalidate 样本，Host p95 为 `17.284/16.895/17.208ms`，Rust apply p95 为 `0.851/0.708/0.687ms`，端到端 p95 为 `17.947/17.466/17.825ms`；RSS p95 为 `9.40/9.59/10.33MiB`，均在 B4 预算内。Bridge diagnostics 仅给出 component ID、代数、单调时间、revision、计数和数值 final state，ring 上限 10,000；Rust trace 记录 `extension_component_frame_applied` 的 componentId/revision/单调时间，无 component lines。严格 verifier 与负向脚本测试通过；`check:rust-spike` 新增一轮 `80x24` storm smoke，Extended Quality 只在手工 `rust-component-benchmark` 入口运行完整矩阵。详见 `docs/rust-tui-b4-extension-report.md`。Windows named pipe、Kitty/iTerm2 实机图片终端和真实外部 Provider 仍未验证。
