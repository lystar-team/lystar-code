...
- GUI Release 不再监听 `gui-preflight/**` 分支，避免同一 commit 先跑一次五平台预检、打 tag 后再完整重跑；手动 `workflow_dispatch` 仍保留为显式预检入口，Release job 继续只允许 `refs/tags/gui-v*`。五平台 Rust registry/git/target 增加按 OS、架构、Rust 版本和 `Cargo.lock` 分组的 Actions cache。`gui.5` 正式发布矩阵约 14 分钟；`gui.6` 的实际耗时必须等候选 workflow 跑完后再记录。
