...
最新补充：Rust TUI 已接入本地只读 `/hotkeys` 详情页，并加入 Ctrl+P 命令面板和 `/help`。页面按导航、编辑、工作台和输入前缀分组，只列 Rust 输入处理器当前实际生效的按键；未展示尚未迁移的 `!`/`!!`，也未新增 Host/Protocol 链路。本轮定向测试 `2/2`、Rust TUI lib 测试 `91/91`、正式 binary check、Clippy `-D warnings`、fmt、`git diff --check` 和根级 `NODE_TLS_REJECT_UNAUTHORIZED=1 npm run check` 均通过；CodeGraph affected 仅指向 changelog/hotkeys 测试。Rust 当前仍不读取 `keybindings.json`，Extension 动态快捷键也未投影到协议；PTY/E2E、macOS、Windows 和正式 `lc` 启动链未在本轮验证。
