...
最新补充：Rust B3 会话工作台专项在 Linux x64 完成 Host-Rust tmux/FIFO 全量 E2E 连续两轮，各 `11/11`，每轮使用独立临时 Session、FIFO、tmux socket 和 artifact。会话切换覆盖 A->B 的严格 release/acquire、B 失败回取 A、双失败后自动进入带 `RecoverySession` 来源的 Sessions chooser；此时 active session 和 lease 均为空，`q` 直接退出，普通用户打开的 Sessions 仍将 `q` 用作筛选，Esc 关闭，Ctrl+C 继续走全局 abort 语义。退出后显式关闭 Host connection，Host `releaseClient` 清理路径、lease count 为 0 与 `stty -g` 恢复均已核验。设置写入与刷新并行回包按请求 ID 应用，不再由全局 generation 误丢；Tree 已完成两轮外部矩阵，覆盖 filter、label/clear、navigate、summary、editor replace、fork、错误和掉回包重试。`npm run benchmark:rust-b3-workbench` 生成 75 条（5 场景 x 3 尺寸 x 5 轮），最新最大 p95/p99 为 5.555ms，RSS p95 为 25.805MiB，缓存两侧固定 400 rounds，regroup 不变；本项只完成 B3 会话工作台范围，不表示 B3 总体完成。详见 `docs/rust-tui-b3-workbench-report.md`。
